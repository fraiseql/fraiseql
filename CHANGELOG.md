...
### Fixed
- **Nested Object Instantiation**: Fixed issue where nested objects from JSONB data remained as dictionaries instead of being instantiated as typed objects
  - Enhanced `from_dict` method to recursively instantiate nested FraiseQL types
  - Added support for Python 3.10+ Union type syntax (`T | None`)
  - UUID fields are now properly converted from strings to UUID objects
  - GraphQL field resolvers now defensively convert dictionaries to typed objects when needed
  - Fixes GraphQL type mismatch errors when querying nested objects from JSONB data
...
### Fixed
- **Critical Bug Fix**: PostgreSQL SET LOCAL commands no longer use prepared statements
  - Fixed `psycopg.errors.SyntaxError: syntax error at or near "$1"` when using query timeouts
  - SET LOCAL statement_timeout now uses literal values instead of parameterized queries
  - This was causing integration test failures for users upgrading to v0.1.0b1

### Added
- Comprehensive tests for SET LOCAL statement execution
- Documentation for the prepared statement fix

### Technical Details
- Changed from `SET LOCAL statement_timeout = %s` with parameters to `SET LOCAL statement_timeout = '30000ms'` with literal values
- PostgreSQL doesn't support parameterized queries for SET/SET LOCAL commands
- The fix maintains security by ensuring timeout values are always integers
