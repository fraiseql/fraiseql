"""FraiseQL Repository - Primary user-facing database access API.

This module provides FraiseQLRepository, the main interface for database access.
It coordinates between the Python API layer and the Rust execution pipeline.

Philosophy: "Python API Exposure + Rust Core"
- Users interact with a clean Python API (FraiseQLRepository)
- Internal Rust coordination is isolated in executor.py
- Clear separation between Python logic and Rust execution engine

Public Methods (User-Facing API):
- find(): Query multiple records with filtering
- find_one(): Query single record
- count(): Count matching records
- exists(): Check if records exist
- sum(), avg(), min(), max(): Aggregations
- distinct(): Get distinct values
- pluck(): Extract single field values
- aggregate(): Complex aggregations
- batch_exists(): Check multiple conditions
- run(): Execute raw queries
- run_in_transaction(): Execute with transactions
- execute_function(): Call PostgreSQL functions

Internal Coordination (Delegated):
- Uses query_builder for SQL generation
- Uses session for RLS variables
- Uses registry for type management
- Uses executor for Rust pipeline
- Uses pool for database connections
"""
