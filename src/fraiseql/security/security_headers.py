...
class ContentSecurityPolicy:
    ...
    def to_header_value(self) -> str:
        ...
        for directive, sources in self.directives.items():
            if directive in [CSPDirective.UPGRADE_INSECURE_REQUESTS, 
...
class SecurityHeadersConfig:
    ...
    # X-Frame-Options
    frame_options: Optional[FrameOptions] = FrameOptions.SAMEORIGIN
    ...
    # Referrer-Policy
    referrer_policy: Optional[ReferrerPolicy] = ReferrerPolicy.STRICT_ORIGIN_WHEN_CROSS_ORIGIN
    ...
    cross_origin_embedder_policy: Optional[str] = None  # "require-corp" or "unsafe-none"
    cross_origin_opener_policy: Optional[str] = None    # "same-origin", "same-origin-allow-popups", "unsafe-none"
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def _add_security_headers(self, request: Request, response: Response):
        ...
        if self.config.frame_options:
            ...
            if (self.config.frame_options == FrameOptions.ALLOW_FROM and 
                self.config.frame_options_allow_from):
...
def create_development_csp() -> ContentSecurityPolicy:
    ...
    csp.add_directive(CSPDirective.CONNECT_SRC, [
        "'self'", 
        "http://localhost:*", 
        "ws://localhost:*",
        "wss://localhost:*"
    ...
    csp.add_directive(CSPDirective.FONT_SRC, [
        "'self'", 
...
def create_csp_report_handler(webhook_url: Optional[str] = None):
    ...
    async def csp_report_endpoint(request: Request):
        ...
        try:
            ...
            print(f"CSP Violation: {violation.get('violated-directive')} "
...
def add_csp_reporting(app, report_uri: str, webhook_url: Optional[str] = None):
