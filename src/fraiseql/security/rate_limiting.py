...
class RateLimitRule:
    ...
    exempt_func: Optional[Callable[[Request], bool]] = None
...
class GraphQLRateLimiter:
    ...
    def __init__(self, store: Union[RateLimitStore, RedisRateLimitStore]):
        self.store = store
    ...
    def _extract_operation_info(self, request_body: dict) -> tuple[str, Optional[str], int]:
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        self, 
        app,
        store: Optional[Union[RateLimitStore, RedisRateLimitStore]] = None,
    ...
    async def _apply_rate_limits(self, request: Request) -> Optional[Response]:
    ...
    async def _is_exempt(self, request: Request) -> bool:
        ...
        for rule in self.rules:
            if (rule.exempt_func and 
                self._matches_pattern(request.url.path, rule.path_pattern) and
                rule.exempt_func(request)):
                return True
