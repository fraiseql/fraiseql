    ...
    def generate_view(self, cls: type, options: Optional[ViewOptions] = None) -> str:
        ...
        if options.joins:
            for join in options.joins:
                ...
                join_fields.append(f"        '{join['field']}', jsonb_build_object(\n{',\\n'.join(join_obj_fields)}\n        )")
...
ORDER BY {order_clause};"""
    ...
    @staticmethod
    def relationship(parent_table: str, child_table: str, relationship_field: str, foreign_key: str) -> str:
...
class SQLHelper:
    ...
    def validate_sql(self, sql: str) -> ValidationResult:
        ...
        if "create view" in sql.lower() and not re.search(r"create\s+(or\s+replace\s+)?view\s+v_\w+", sql.lower()):
