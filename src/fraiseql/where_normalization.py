...
Phase 7.2: Rust WHERE normalization integration
- Rust implementation is 7-10x faster than Python
- Backward compatible via WhereClause wrapper
- Feature flag: USE_RUST_WHERE_NORMALIZATION (default: True)
"""
...
def normalize_dict_where(
    ...
    # Phase 7.2: Try Rust implementation first (7-10x faster)
    if USE_RUST_WHERE_NORMALIZATION:
        try:
            from fraiseql._fraiseql_rs import normalize_where_to_sql
            from fraiseql.db import _table_metadata

            # Get FK mappings from metadata
            fk_mappings = {}
            if view_name in _table_metadata:
                metadata = _table_metadata[view_name]
                fk_mappings = metadata.get("fk_relationships", {})

            # Call Rust WHERE normalization
            sql, params = normalize_where_to_sql(
                json.dumps(where_dict),  # WHERE dict as JSON
                list(table_columns) if table_columns else [],  # Table columns
                json.dumps(fk_mappings),  # FK mappings as JSON
                jsonb_column,  # JSONB column name
            )

            # Wrap in WhereClause for backward compatibility
            # The Rust implementation directly generates SQL, so we create a
            # WhereClause that just returns the pre-generated SQL
            return _create_whereclause_from_sql(sql, params)
...
def _create_whereclause_from_sql(sql: str, params: list[str]) -> WhereClause:
    """Create a WhereClause from pre-generated SQL (Phase 7.2 Rust integration).

    This wrapper allows the Rust WHERE normalization to integrate seamlessly
    with the Python WhereClause API. The SQL is already generated by Rust,
    so we create a WhereClause that returns it verbatim.

    Args:
        sql: Pre-generated SQL WHERE clause (e.g., "status = $1 AND category = $2")
        params: List of parameter values (e.g., ["active", "books"])

    Returns:
        WhereClause that returns the pre-generated SQL

    Note:
        The returned WhereClause is a thin wrapper - it doesn't re-process
        the SQL, it just returns what Rust already generated. This provides
        7-10x performance improvement while maintaining API compatibility.
    """
    from psycopg.sql import SQL

    # Create a simple WhereClause that returns the pre-generated SQL
    # We use a single RawSQL condition to wrap the Rust-generated SQL
    clause = WhereClause(conditions=[], nested_clauses=[], logical_op="AND")

    # Monkey-patch the to_sql method to return Rust-generated SQL
    # This avoids having to extend WhereClause or FieldCondition
    def _rust_to_sql():
        return (SQL(sql), params)

    clause.to_sql = _rust_to_sql
    return clause
