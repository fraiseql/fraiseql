...
class WhereClauseMixin:
    ...
    def _validate_value(self, field_name: str, value: Any) -> Any:
        ...
        # List validation
            ...
            return [self._validate_value(f"{field_name}[{i}]", item) for i, item in enumerate(value)]
    ...
    def _build_condition(self, field_name: str, operator: str, value: Any, field_type: type) -> Composed:
    ...
    def to_sql(self) -> Optional[Composed]:
...
def create_where_type(cls: type, config: Optional[WhereGeneratorConfig] = None) -> type:
