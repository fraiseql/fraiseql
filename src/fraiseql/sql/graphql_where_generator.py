...
from fraiseql.sql.where.operators.rich_type_filters import (
    AirportCodeFilter,
    ApiKeyFilter,
    ColorFilter,
...
def _get_filter_type_for_field(
    ...
    # Check for rich scalar types (44 specialized scalar types)
    # These provide specialized filtering for domain-specific data
    try:
        ...
        rich_scalar_mapping = {
            EmailAddressField: EmailAddressFilter,
            PhoneNumberField: PhoneNumberFilter,
            URLField: URLFilter,
            DomainNameField: DomainNameFilter,
            HostnameField: HostnameFilter,
            PostalCodeField: PostalCodeFilter,
            LatitudeField: LatitudeFilter,
            LongitudeField: LongitudeFilter,
            CoordinateField: CoordinatesFilter,
            TimezoneField: TimezoneFilter,
            LocaleCodeField: LocaleCodeFilter,
            LanguageCodeField: LanguageCodeFilter,
            # ISO identifiers
            IBANField: IbanFilter,
            CUSIPField: CusipFilter,
            ISINField: IsinFilter,
            SEDOLField: SedolFilter,
            LEIField: LeiFilter,
            MICField: MicFilter,
            # Financial instruments
            CurrencyCodeField: CurrencyCodeFilter,
            MoneyField: MoneyFilter,
            ExchangeCodeField: ExchangeCodeFilter,
            ExchangeRateField: ExchangeRateFilter,
            StockSymbolField: StockSymbolFilter,
            # Text and content
            SlugField: SlugFilter,
            SemanticVersionField: SemanticVersionFilter,
            HashSHA256Field: HashSha256Filter,
            ApiKeyField: ApiKeyFilter,
            # Transportation and logistics
            LicensePlateField: LicensePlateFilter,
            VINField: VinFilter,
            TrackingNumberField: TrackingNumberFilter,
            ContainerNumberField: ContainerNumberFilter,
            # Network and geography
            IpAddressField: IpAddressFilter,
            CIDRField: CidrFilter,
            PortField: PortFilter,
            AirportCodeField: AirportCodeFilter,
            PortCodeField: PortCodeFilter,
            FlightNumberField: FlightNumberFilter,
            # Content types
            MarkdownField: MarkdownFilter,
            HTMLField: HtmlFilter,
            MimeTypeField: MimeTypeFilter,
            ColorField: ColorFilter,
            ImageField: ImageFilter,
            FileField: FileFilter,
            # Ranges and measurements
            DateRangeField: DateRangeTypeFilter,
            DurationField: DurationFilter,
            PercentageField: PercentageFilter,
        }

        if field_type in rich_scalar_mapping:
            return rich_scalar_mapping[field_type]

    except ImportError:
        # Rich scalar types not available, continue with standard mapping
        pass
