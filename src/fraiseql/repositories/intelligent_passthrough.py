...
class AutoOptimizedRepository(IntelligentPassthroughMixin):
    ...
    async def find(self, *args, **kwargs):
        ...
        if hasattr(self, '_can_use_raw_passthrough') and self._can_use_raw_passthrough(args[0] if args else '', **kwargs):
...
async def example_usage(repo: AutoOptimizedRepository):
    ...
    user = await repo.find_one("v_user", id="user-123")
    
    # Complex query with custom processing → Python processing (when needed)
    complex_users = await repo.find("v_user", where=ComplexWhereClause())
    
    # Get optimization stats
    stats = repo.get_performance_stats()
    print(f"Optimized {stats['raw_passthrough_count']} of {stats['total_queries']} queries")