...
public sealed class MutationBuilder
{
    private static readonly HashSet<string> ValidOperations =
        new(StringComparer.OrdinalIgnoreCase) { "create", "insert", "update", "delete", "upsert" };
    ...
    /// </summary>
    /// <param name="operation">One of <c>"create"</c>, <c>"insert"</c>, <c>"update"</c>, <c>"delete"</c>, or <c>"upsert"</c> (case-insensitive).</param>
    /// <returns>This builder for chaining.</returns>
    ...
    public MutationBuilder Operation(string operation)
    {
        if (!ValidOperations.Contains(operation))
            throw new ArgumentException(
                $"Invalid operation '{operation}'. Must be one of: create, insert, update, delete, upsert.",
                nameof(operation));
    ...
    /// <summary>Sets the HTTP method for the REST endpoint. Defaults to <c>"POST"</c> when omitted.</summary>
    /// <param name="method">The HTTP method (e.g. <c>"POST"</c>, <c>"PUT"</c>). Case-insensitive, stored uppercase.</param>
    /// <returns>This builder for chaining.</returns>
    public MutationBuilder RestMethod(string method) { _restMethod = method.ToUpperInvariant(); return this; }
    ...
    public IntermediateMutation Build()
    {
        ...
        RestAnnotation? rest = _restPath is not null
            ? new RestAnnotation(_restPath, _restMethod ?? "POST")
            : null;
