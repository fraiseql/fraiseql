...
def test_query_sql_source_dispatch_explicit_mapping() -> None:
    ...
    @fraiseql.query(sql_source_dispatch={"timeInterval": {"DAY": "tf_orders_day", "WEEK": "tf_orders_week", "MONTH": "tf_orders_month"}})
...
def test_query_sql_source_dispatch_mutual_exclusivity_with_sql_source() -> None:
    ...
    with pytest.raises(ValueError, match="cannot set sql_source together with sql_source_dispatch"):
        @fraiseql.query(sql_source="v_orders", sql_source_dispatch={"timeInterval": {"DAY": "tf_orders_day"}})
...
def test_query_sql_source_dispatch_mutual_exclusivity_with_template() -> None:
    ...
    with pytest.raises(ValueError, match="cannot set both sql_source_dispatch and sql_source_template"):
        @fraiseql.query(sql_source_dispatch={"timeInterval": {"DAY": "tf_orders_day"}}, sql_source_template="tf_orders_{time_interval}")
        def orders(timeInterval: str) -> list[dict]:
            pass
...
def test_query_sql_source_template_no_placeholder() -> None:
    ...
    with pytest.raises(ValueError, match="sql_source_template must contain exactly one \\{placeholder\\}"):
...
def test_query_sql_source_template_placeholder_not_in_args() -> None:
    ...
    with pytest.raises(ValueError, match="template placeholder 'missing' does not match any argument"):
