...
module MutationBuilder =
    ...
    /// Sets the REST path for this mutation.
    let restPath (p: string) (s: MutationState) : MutationState = { s with restPath = Some p }

    /// Sets the REST HTTP method for this mutation.
    let restMethod (m: string) (s: MutationState) : MutationState = { s with restMethod = Some m }
    ...
    let toDefinition (s: MutationState) : MutationDefinition =
...

        {
            ...
            rest =
                s.restPath
                |> Option.map (fun p ->
                    { path = p; method = s.restMethod |> Option.defaultValue "POST" })
        }
