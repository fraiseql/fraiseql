...
|-----------|----------|--------------|-------------|---------------|
| `fraiseql-python/` | Python 3.11+ | PyPI | 26/26 | — |
| `fraiseql-typescript/` | TypeScript / Node.js | npm | 26/26 | — |
| `fraiseql-go/` | Go 1.23+ | — (source-only) | 25/26 | type_crud: the SDK ships no CRUD generator |
| `fraiseql-php/` | PHP 8.2+ | — (source-only) | 26/26 | — |
| `fraiseql-java/` | Java 21+ | — (source-only) | 26/26 | — |
| `fraiseql-csharp/` | C# / .NET 8+ | — (source-only) | 25/26 | subscriptions: the SDK ships no subscription authoring surface at all (#1024) |
| `fraiseql-fsharp/` | F# / .NET 8+ | — (source-only) | 25/26 | subscriptions: the SDK ships no subscription authoring surface at all (#1024) |
| `fraiseql-elixir/` | Elixir | — (source-only) | 25/26 | subscriptions: the SDK ships no subscription authoring surface at all (#1024) |
| `fraiseql-ruby/` | Ruby 3.2+ | — (source-only) | 25/26 | subscriptions: the SDK ships no subscription authoring surface at all (#1024) |
| `fraiseql-dart/` | Dart / Flutter | — (source-only) | 25/26 | subscriptions: the SDK ships no subscription authoring surface at all (#1024) |
| `fraiseql-rust/` | Rust | crates.io | 5/26 | queries, mutations, subscriptions, enums, input types, CRUD, relationships, Relay and error-type flags, serverless functions: the Rust SDK is field-level-RBAC focused and ships no builder for them |
