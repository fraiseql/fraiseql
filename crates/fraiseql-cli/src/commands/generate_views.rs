...
/// - `--validate` is given and the generated DDL fails to execute against `PostgreSQL` (or no
///   `DATABASE_URL` is available to execute it)
/// - Output file can't be written
pub async fn run(config: GenerateViewsConfig, formatter: &OutputFormatter) -> Result<()> {
    if config.verbose {
    ...
    // 6. If validate-only, execute the DDL for real and stop here. A validator that inspects
    //    nothing cannot fail — the previous implementation printed "View DDL is valid" for files
    //    PostgreSQL rejected (#821).
    if config.validate_only {
        validate_ddl_against_database(&sql).await?;
        println!("✓ View DDL is valid (executed against PostgreSQL, rolled back)");
...
/// Execute the generated DDL inside a transaction that is always rolled back.
///
/// This is what makes `--validate` honest: the statements run against a real
/// `PostgreSQL` (the same engine that will execute the file), so a syntax
/// error, a missing source relation, or a column mismatch fails the command.
/// Nothing is committed. Requires `DATABASE_URL`, like `fraiseql setup`.
///
/// # Errors
///
/// Returns an error when `DATABASE_URL` is unset, the connection fails, or any
/// statement in the generated DDL is rejected by the database.
async fn validate_ddl_against_database(sql: &str) -> Result<()> {
    let db_url = std::env::var("DATABASE_URL").map_err(|_| {
        anyhow::anyhow!(
            "--validate executes the generated DDL against PostgreSQL and needs DATABASE_URL \
    ...
    txn.batch_execute(sql)
        .await
        .context("generated DDL was rejected by PostgreSQL — the file is NOT valid")?;
    txn.rollback().await.context("--validate: rollback failed")?;
    Ok(())
}
...
pub(crate) fn generate_view_sql(
    ...
    // Drop existing view if it exists. The drop must match the object class:
    // tv_ targets are MATERIALIZED views, and a plain `DROP VIEW` on one errors
    // ("… is not a view"), making the file non-idempotent (#821 adjunct).
    if view_name.starts_with("tv_") {
        sql.push_str(&format!("DROP MATERIALIZED VIEW IF EXISTS {view_name} CASCADE;\n\n"));
    } else {
    ...
    if include_composition_views {
        sql.push_str("\n-- Composition views\n");
        generate_composition_views(&mut sql, view_name);
    }
...
///
/// They read the **generated** view (not the entity source), so the main
/// view's `DROP … CASCADE` removes them on re-run and the whole file stays
/// idempotent. A previous version emitted the literal placeholder `{}` for the
/// relation name — both statements were PostgreSQL syntax errors on every
/// invocation, and `--validate` reported the file valid anyway (#821).
fn generate_composition_views(sql: &mut String, view_name: &str) {
    let base_name = view_name
        .trim_start_matches("va_")
        .trim_start_matches("tv_")
        .trim_start_matches("ta_");

    // The tv_ view exposes `materialized_at` (see `generate_table_vector_view`);
    // va_/ta_ views project the source's `updated_at`.
    let ts_column = if view_name.starts_with("tv_") {
        "materialized_at"
    } else {
        "updated_at"
    };
    ...
    sql.push_str(&format!("SELECT * FROM {view_name}\n"));
    sql.push_str(&format!("WHERE {ts_column} > NOW() - INTERVAL '7 days'\n"));
    sql.push_str(&format!("ORDER BY {ts_column} DESC;\n\n"));

    // Count view
    sql.push_str(&format!("CREATE VIEW {base_name}_count AS\n"));
    sql.push_str(&format!("SELECT COUNT(*) as total FROM {view_name};\n"));
}

/// Generate monitoring functions for the view.
///
/// STABLE, not IMMUTABLE: the function reads a relation, and IMMUTABLE would
/// license the planner to constant-fold the row count inside a cached or
/// prepared plan (#821 adjunct).
fn generate_monitoring_functions(sql: &mut String, view_name: &str) {
    ...
    sql.push_str("END;\n");
    sql.push_str("$$ LANGUAGE plpgsql STABLE;\n");
}
