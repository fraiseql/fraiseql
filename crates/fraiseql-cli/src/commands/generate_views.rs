...
impl RefreshStrategy {
    ...
    pub fn from_str(s: &str) -> std::result::Result<Self, String> {
        match s.to_lowercase().as_str() {
            ...
            _ => Err(format!(
                "Invalid refresh strategy '{s}', expected: trigger-based, scheduled"
            )),
        }
    }
}
...
pub fn run(config: GenerateViewsConfig) -> Result<()> {
    ...
    let output_path = config.output.unwrap_or_else(|| {
        format!("{}.sql", config.view)
    });

    fs::write(&output_path, sql.clone())
...
fn validate_entity(schema: &CompiledSchema, entity: &str) -> Result<()> {
    if schema.types.iter().any(|t| t.name == entity) {
        ...
        let available = schema
            ...
            .join(", ");
        anyhow::bail!(
            "Entity '{}' not found in schema. Available types: {}",
            entity,
            available
        )
    }
...
fn validate_view_name(view_name: &str) -> Result<&'static str> {
    if view_name.starts_with("va_") {
        ...
        anyhow::bail!(
            "Invalid view name '{}'. Must start with va_, tv_, or ta_",
            view_name
        )
    }
}
...
fn generate_table_vector_view(sql: &mut String, entity: &str, view_name: &str) {
    ...
    sql.push_str(&format!(
        "CREATE INDEX idx_{base_name}_id ON {view_name} (id);\n"
    ));
}
...
fn generate_composition_views(sql: &mut String, _entity: &str, view_name: &str) {
    ...
    // Recent items view
    sql.push_str(&format!(
        "CREATE VIEW {base_name}_recent AS\n"
    ));
    ...
    // Count view
    sql.push_str(&format!(
        "CREATE VIEW {base_name}_count AS\n"
    ));
...
mod tests {
    ...
    fn test_refresh_strategy_from_str() {
        ...
        assert_eq!(
            RefreshStrategy::from_str("trigger").unwrap(),
            RefreshStrategy::TriggerBased
        );
        assert_eq!(
            RefreshStrategy::from_str("scheduled").unwrap(),
    ...
    fn test_validate_view_name_vector_arrow() {
        assert_eq!(
            validate_view_name("va_user_embeddings").unwrap(),
            "Vector Arrow (va_)"
        );
    }
    ...
    fn test_validate_view_name_table_vector() {
        assert_eq!(
            validate_view_name("tv_user_profile").unwrap(),
            "Table Vector (tv_)"
        );
    }
    ...
    fn test_validate_view_name_table_arrow() {
        assert_eq!(
            validate_view_name("ta_orders").unwrap(),
            "Table Arrow (ta_)"
        );
    }
