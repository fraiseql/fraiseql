        ...
        for cap in type_start_re.captures_iter(source) {
            ...
            if let Some(body) = extract_balanced_braces(&source[after_match..]) {
                ...
                types.push(IntermediateType {
                    ...
                    // `extract` scrapes source text; no language has a relationship annotation for
                    // it to find, and it already omits `inject_params` and
                    // `subscribable_tables` on the same grounds. Authoring one
                    // goes through the SDK or `fraiseql.toml` (#1266).
