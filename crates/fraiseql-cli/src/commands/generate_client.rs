...
pub enum ClientLanguage {
    ...
    TypeScript,
    /// Python client (TypedDicts + typed query/mutation functions; ≥ 3.12).
    Python,
    /// Go client (structs + typed query/mutation functions; ≥ 1.21).
    Go,
    /// Rust client (serde types + typed query/mutation functions).
    Rust,
}
...
pub fn run(
    ...
    let mut files = match language {
        ...
        ClientLanguage::Python => fraiseql_codegen::client::python::generate(&schema)
            .map_err(|e| anyhow::anyhow!("client generation failed: {e}"))?,
        ClientLanguage::Go => fraiseql_codegen::client::go::generate(&schema)
            .map_err(|e| anyhow::anyhow!("client generation failed: {e}"))?,
        ClientLanguage::Rust => fraiseql_codegen::client::rust::generate(&schema)
            .map_err(|e| anyhow::anyhow!("client generation failed: {e}"))?,
    };
    ...
    match language {
        ClientLanguage::TypeScript => println!(
            "\nGenerated {} files into {}. Type-check with `tsc --strict --noEmit {}/index.ts`.",
            files.len(),
            out_dir.display(),
            out_dir.display(),
        ),
        ClientLanguage::Python => println!(
            "\nGenerated {} files into {}. Type-check with `ty check {}` (Python >= 3.12).",
            files.len(),
            out_dir.display(),
            out_dir.display(),
        ),
        ClientLanguage::Go => println!(
            "\nGenerated {} files into {} (package `fraiseqlclient`, Go >= 1.21). Compile with `go build ./...`.",
            files.len(),
            out_dir.display(),
        ),
        ClientLanguage::Rust => println!(
            "\nGenerated {} files into {}. Declare it (`pub mod {};`) and compile with `cargo check`; \
             the client needs `serde` (with `derive`) and `serde_json`.",
            files.len(),
            out_dir.display(),
            out_dir
                .file_name()
                .map_or_else(|| "generated".into(), std::ffi::OsStr::to_string_lossy),
        ),
    }
    Ok(())
}
...
/// auto-generated sentinel in each language's `types` file).
fn contains_generated_client(out_dir: &Path) -> bool {
    ["types.ts", "types.py", "types.go", "types.rs"].iter().any(|marker| {
        std::fs::read_to_string(out_dir.join(marker))
