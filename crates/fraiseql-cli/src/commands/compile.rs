...
mod tests {
    ...
    /// Minimal valid schema.json that the full compile pipeline accepts.
    const MINIMAL_SCHEMA_JSON: &str = r#"{
        "types": [{
            "name": "User",
            "fields": [
    ...
    #[tokio::test]
    async fn test_compile_to_schema_returns_correct_counts() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("schema.json");
        std::fs::write(&path, MINIMAL_SCHEMA_JSON).unwrap();

        let (schema, _report) = super::compile_to_schema(
            path.to_str().unwrap(),
            None,
            None,
            vec![],
        ...
        assert_eq!(schema.types.len(), 1, "expected 1 type");
        assert_eq!(schema.queries.len(), 1, "expected 1 query");
        assert_eq!(schema.mutations.len(), 0, "expected 0 mutations");
    }

    #[tokio::test]
    async fn test_compile_to_schema_preserves_type_name() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("schema.json");
        std::fs::write(&path, MINIMAL_SCHEMA_JSON).unwrap();
    ...
    #[tokio::test]
    async fn test_compile_to_schema_optimization_report_is_non_empty_for_list_query() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("schema.json");
        // List query with an argument triggers an index hint in the optimizer
        let schema_with_arg = r#"{
            "types": [{"name": "Post", "fields": [{"name": "id", "type": "Int", "nullable": false}]}],
            "queries": [{
                "name": "posts",
                "return_type": "Post",
                "returns_list": true,
                "sql_source": "v_post",
                "jsonb_column": "data",
                "arguments": [{"name": "authorId", "type": "Int", "nullable": false}]
            }]
        }"#;
        std::fs::write(&path, schema_with_arg).unwrap();

        let (_schema, report) = super::compile_to_schema(
            path.to_str().unwrap(),
            None,
            None,
            vec![],
        ...
        assert!(report.has_suggestions(), "list query with arg should produce index hint");
    }

    #[tokio::test]
    async fn test_compile_to_schema_missing_file_returns_error() {
        let result = super::compile_to_schema(
    ...
    #[tokio::test]
    async fn test_compile_to_schema_invalid_json_returns_error() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("schema.json");
