...
fn test_rich_filter_compilation_pipeline() {
    ...
    for db in &["postgres", "mysql", "sqlite", "sqlserver"] {
        assert!(
            domain_eq.contains_key(*db),
            "Should have {} template for domainEq",
            db
        );
    }
    ...
    assert!(
        compiled.security.is_some(),
        "Security section should exist for lookup data"
    );
    ...
    assert!(
        lookup.contains_key("countries"),
        "Countries lookup should be present"
    );
    assert!(
        lookup.contains_key("currencies"),
        "Currencies lookup should be present"
    );
    assert!(
        lookup.contains_key("timezones"),
        "Timezones lookup should be present"
    );
    assert!(
        lookup.contains_key("languages"),
        "Languages lookup should be present"
    );
}
...
fn test_where_input_fields_include_standard_operators() {
    ...
    assert!(
        field_names.contains(&&"isnull".to_string()),
        "Should have isnull operator"
    );
...
fn test_sql_templates_cover_all_databases() {
    ...
    let types_to_check = vec!["EmailAddressWhereInput", "VINWhereInput", "CoordinatesWhereInput"];
    ...
    for type_name in types_to_check {
        let where_input = compiled
            ...
            .expect(&format!("{} should exist", type_name));

        let metadata = where_input.metadata.as_ref().expect(&format!(
...
fn test_lookup_data_integrity() {
    ...
    if let Some(us) = countries.get("US") {
        ...
        assert!(
            us_obj.contains_key("continent"),
            "US should have continent"
        );
    ...
    if let Some(usd) = currencies.get("USD") {
        ...
        assert!(
            usd_obj.contains_key("decimal_places"),
            "USD should have decimal_places"
        );
    }
...
fn test_compiled_schema_is_valid() {
    ...
    assert!(!compiled.types.is_empty() || !compiled.input_types.is_empty(),
        "Compiled schema should have types or input types");
    assert!(compiled.input_types.len() > 0, "Should have input types");

    // Verify all WhereInput types have valid structure
    for where_input in compiled.input_types.iter() {
