...
async fn validate_token_rejects_wrong_signing_key() {
    ...
    // Sign a JWT with a *different* RSA key (generate fresh one at runtime
    // using a second hardcoded key that does NOT match the JWKS public key).
    // We reuse the same PEM but tamper with the token payload after signing
    // to break the signature. Simpler: just flip a character in the token.
    let now = chrono::Utc::now().timestamp();
    let claims = json!({
        "sub": "user-42",
        "iss": issuer,
