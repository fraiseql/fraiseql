...
async fn connection_with_wrong_credentials_returns_error() {
    ...
    let bad_url = format!(
        "postgres://wrong_user:wrong_pass@127.0.0.1:{}/testdb",
        container.port
    );
...
async fn raw_query_with_syntax_error_returns_database_error() {
    ...
    let result = adapter
        .execute_raw_query("SELCT * FORM nonexistent")
        .await;
    assert!(result.is_err());
...
async fn query_on_nonexistent_view_returns_database_error() {
    let adapter = common::testcontainer::get_test_adapter().await;
    let result = adapter
        .execute_where_query("v_does_not_exist", None, None, None)
        .await;
    assert!(result.is_err());
    let err = result.unwrap_err();
    assert!(
...
async fn query_timeout_via_statement_timeout() {
    ...
    let _ = adapter
        .execute_raw_query("SET statement_timeout = '10ms'")
        .await;
...
async fn pool_size_one_with_concurrent_queries_does_not_hang() {
    ...
    let (r1, r2) = tokio::join!(
        a1.execute_raw_query("SELECT 1 AS v"),
        a2.execute_raw_query("SELECT 2 AS v"),
    );
...
async fn query_seeded_view_returns_data() {
    let adapter = common::testcontainer::get_test_adapter().await;
    let results = adapter
        .execute_where_query("test.v_user", None, None, None)
        .await
        .unwrap();
    assert!(
        !results.is_empty(),
        "seeded v_user should return rows"
    );
}
