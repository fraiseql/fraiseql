...
//!
//! Comprehensive tests for compile-time and runtime enforcement of @requires and @provides directives.
//! Tests cover validation, error handling, and integration with entity resolution.

use std::collections::HashMap;

use fraiseql_core::federation::types::{
    FieldFederationDirectives, FieldPathSelection, FederatedType, FederationMetadata, KeyDirective,
};
...
fn test_requires_references_valid_field() {
    let order = create_order_with_requires();
    let requires_fields = order
        .get_field_directives("shippingEstimate")
        .unwrap()
        .requires
        .clone();
...
fn test_requires_validation_success_all_fields_present() {
    ...
    let all_present = directives.requires.iter().all(|req| {
        entity_fields.contains_key(&req.path[0])
    });
...
fn test_requires_with_multiple_required_fields() {
    ...
    let all_present = field_directives.requires.iter().all(|req| {
        entity_fields.contains(&req.path[0].as_str())
    });
    assert!(all_present);
}
...
fn test_provides_validates_returned_fields() {
    ...
    let promised_fields: Vec<_> = field_directives
        .provides
        .iter()
...
fn test_provides_warning_if_field_not_returned() {
    ...
    let missing_promised = promised_fields
        .iter()
        .any(|p| !returned_fields.contains(&p.as_str()));
