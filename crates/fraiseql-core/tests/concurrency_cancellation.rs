...
async fn test_query_cancellation_via_tokio_timeout() {
    let adapter = Arc::new(
        FailingAdapter::new().fail_with_error(FailError::Timeout { timeout_ms: 5000 }),
    );
...
async fn test_concurrent_queries_with_one_cancelled() {
    ...
    assert!(
        successes >= 9,
        "at least 9 non-aborted tasks should succeed, got {successes}"
    );
    assert!(
        successes + aborted == 10,
...
async fn test_rapid_spawn_and_cancel_cycle() {
    ...
    assert!(
        count <= 100,
        "query count should not exceed spawn count, got {count}"
    );
    ...
    assert!(
        result.is_ok(),
        "adapter must remain usable after rapid cancel cycles"
    );
}
