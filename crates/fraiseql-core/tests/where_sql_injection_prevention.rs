    ...
    for (op, expected_sql_op) in operators {
        for payload in INJECTION_PAYLOADS {
            ...
            assert!(
                ...
                "{op:?} must use {expected_sql_op}: {sql}"
            );
...
fn test_injection_in_simple_path_generates_safe_sql() {
    ...
    for malicious_field in malicious_paths {
        ...
        assert!(
            sql.contains("= 'safe_value'"),
            "Value comparison must remain intact: {sql}"
        );
    }
}
...
fn test_injection_in_nested_path_generates_safe_sql() {
    ...
    for path in malicious_paths {
        ...
        assert!(
            sql.contains("data#>'{"),
            "Nested path must use JSON path syntax: {sql}"
        );
...
fn test_encoded_payloads_generate_safe_sql() {
    ...
    for payload in encoded_payloads {
        ...
        assert!(
            ...
            "Encoded payload must be preserved as literal: {sql}"
        );
    }
}
...
fn test_is_null_operator_ignores_value_payload() {
    ...
    assert!(
        !sql.contains("DROP"),
        "IsNull must not include value payload in SQL: {sql}"
    );
}
...
fn test_array_operator_injection_generates_safe_sql() {
    ...
    assert!(
        sql.contains("''"),
        "Quotes inside JSON array values must be doubled: {sql}"
    );
}
