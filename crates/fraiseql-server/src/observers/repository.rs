...
impl ObserverRepository {
    ...
    /// # Errors
    ///
    /// Returns a [`ServerError::Database`] error if the database query fails.
    pub async fn list(
        &self,
    ...
    /// # Errors
    ///
    /// Returns a [`ServerError::Database`] error if the database query fails, or
    /// [`ServerError::Conflict`] if an observer with the same name already exists.
    pub async fn create(
        &self,
