...
fn export_schema() -> CompiledSchema {
    ...
    // `auto_params.has_where` comes from the builder now that it follows the compiler
    // (#1290). It matters to `an_export_without_a_dotted_parameter_still_streams`, whose
    // whole subject is a plain `?title=` filter surviving this gate: while the builder
    // left every flag `false`, that filter was accepted, dropped by the read path and
    // answered with the unfiltered view, so the case passed over the behaviour it exists
    // to protect — and since #1283 it is refused outright.
