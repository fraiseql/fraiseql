...
//!
//! - **Startup**: `RedisStateStore::new()` returns `Err(AuthError::ConfigError)`
//!   immediately. The caller is responsible for falling back to
//!   `InMemoryStateStore` or aborting startup.
//!
//! - **Mid-operation**: If the Redis connection drops after a successful
//!   `new()`, subsequent `store()` and `retrieve()` calls propagate the
//!   Redis error as `AuthError::ConfigError` (store) or
//!   `AuthError::InvalidState` / `AuthError::ConfigError` (retrieve).
//!   There is no automatic fallback to in-memory — the error bubbles up.
//!
//! - **Recovery**: `redis::aio::ConnectionManager` automatically reconnects
//!   on the next command after a transient failure. No manual intervention
//!   is needed; the next `store()` / `retrieve()` call will succeed once
...
use fraiseql_auth::error::AuthError;
use fraiseql_auth::state_store::{RedisStateStore, StateStore};
...
async fn redis_unavailable_at_startup_returns_config_error() {
    ...
    assert!(
        result.is_err(),
        "connecting to a dead Redis URL must fail, got Ok"
    );
...
async fn redis_invalid_url_returns_config_error() {
    ...
    assert!(
        result.is_err(),
        "invalid Redis URL must fail, got Ok"
    );
...
async fn redis_store_and_retrieve_works_when_healthy() {
    ...
    let store = RedisStateStore::new(&url)
        .await
    ...
    assert!(
        replay.is_err(),
        "replayed retrieve() must fail after consumption"
    );
}
...
async fn redis_store_propagates_error_not_panic() {
    ...
    assert!(
        result.is_ok(),
        "store() must succeed with live Redis: {result:?}"
    );
}
