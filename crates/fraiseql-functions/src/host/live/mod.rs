...
impl HostContext for LiveHostContext {
    ...
    async fn http_request(
        ...
        // One implementation of the outbound op, shared with the request-serving
        // host (#1329). The SSRF allowlist, the literal-IP and DNS-rebinding checks,
        // the disabled redirect policy and the response-size ceiling are the same
        // code for both, not the same rule written twice — #1329's constraint was
        // "the SSRF allowlist, unchanged", and a second copy is how it changes.
