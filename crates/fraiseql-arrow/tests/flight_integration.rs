...
mod tests {
    ...
    // Six `*_unimplemented` tests stood here — for Handshake, DoPut, DoAction,
    // DoExchange, PollFlightInfo and GetFlightInfo. Every one asserted
    // `service.schema_registry().contains("…")`, a constant `register_defaults()`
    // guarantees, and none ever called the RPC it was named for: `rg '\.do_put\('`
    // was zero workspace-wide.
    //
    // That is why the #953 upload allow-list landed on `do_exchange` and not on its
    // sibling `do_put`, and stayed missing through a release — the suite that should
    // have shown the gap could not fail (#1028). The upload decision is now pinned by
    // unit tests in `flight_server::handlers::upload_guard`, and
    // `tools/check-guard-parity.sh` fails if any Flight handler builds an INSERT
    // without calling that guard.
}
