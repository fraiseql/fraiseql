...
impl EventStorage for MockEventStorage {
    async fn query_events(
        ...
        Ok(self.events.iter().filter(|e| e.entity_type == entity_type).cloned().collect())
    }
    ...
    async fn count_events(
        ...
        Ok(self.events.iter().filter(|e| e.entity_type == entity_type).count())
    }
}
...
async fn test_event_storage_count() {
    ...
    let count = storage.count_events("Order", None, None).await.expect("should count events");
