...
impl SagaCoordinator {
    ...
    /// and trusts the configured root CA (see [`fraiseql_federation::tls::MtlsConfig`]), so saga
    ...
    /// (`https_only`, redirect-none, DNS rebinding guard); pass an
    /// [`fraiseql_federation::tls::MtlsConfig`] to present a client certificate to the owning
    /// subgraph.
    ///
    /// # Errors
    ///
    ...
    /// adapter. The SSRF guard ([`fraiseql_federation::http_resolver::validate_subgraph_url`]) is
    ...
    pub fn with_subgraph(mut self, name: impl Into<String>, url: Url) -> Result<Self> {
        fraiseql_federation::http_resolver::validate_subgraph_url(url.as_str())?;
