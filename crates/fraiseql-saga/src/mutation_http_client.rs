...
use fraiseql_federation::{metadata_helpers::find_federation_type, types::FederationMetadata};
...
impl HttpMutationClient {
    ...
    /// [`fraiseql_federation::tls::MtlsMaterial`]). When `mtls.enabled` is false this is a no-op —
    ...
    pub fn new_with_mtls(
        config: HttpMutationConfig,
        mtls: &fraiseql_federation::tls::MtlsConfig,
    ) -> Result<Self> {
    ...
    fn build_client(
        config: &HttpMutationConfig,
        https_only: bool,
        mtls: Option<&fraiseql_federation::tls::MtlsConfig>,
        ...
        if let Some(mtls_cfg) = mtls {
            builder = fraiseql_federation::tls::MtlsMaterial::load(mtls_cfg)?.apply(builder)?;
        }
    ...
    /// [`fraiseql_federation::http_resolver::HttpEntityResolver::new_for_test`].
    ///
    ...
    pub async fn execute_mutation(
        ...
        {
            fraiseql_federation::http_resolver::validate_subgraph_url(subgraph_url)?;
            fraiseql_federation::http_resolver::dns_resolve_and_check(subgraph_url).await?;
        }
        #[cfg(any(test, feature = "test-utils"))]
        if !self.skip_ssrf {
            fraiseql_federation::http_resolver::validate_subgraph_url(subgraph_url)?;
    ...
    pub fn build_mutation_query(
        ...
        mutation_name: &str,
        variables: &Value,
        fed_type: &fraiseql_federation::types::FederatedType,
