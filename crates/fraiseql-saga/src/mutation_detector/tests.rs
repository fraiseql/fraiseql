...
fn test_mutation_ownership_federation_disabled() {
    let metadata = fraiseql_federation::FederationMetadata::default();
...
fn test_mutation_ownership_local_type() {
    let metadata = fraiseql_federation::FederationMetadata {
        enabled: true,
        version: "v2".to_string(),
        types: vec![fraiseql_federation::FederatedType {
            name:                "User".to_string(),
            keys:                vec![],
