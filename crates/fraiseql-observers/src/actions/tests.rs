...
async fn execute_signs_the_exact_transmitted_bytes() {
    // These dispatch to a wiremock on 127.0.0.1, which the SSRF guard blocks;
    // pin the bypass decision for this test (#907 — a pinned decision instead
    // of mutating process env, which raced every bare guard reader).
...
async fn execute_rejects_an_invalid_http_method() {
    ...
    let err = WebhookAction::new()
        .execute(&server.uri(), Some("not a method"), &HashMap::new(), None, None, &test_event())
        .await
