...
/// Pure recovery-phase decision helpers (always compiled; see the module docs for
/// why the logic lives outside the feature gate).
mod recovery;
...
mod wired {
    ...
    use crate::{
        ...
        saga_store::{Result as SagaStoreResult, Saga, SagaStoreError},
    };
    ...
    impl SagaRecoveryManager {
        ...
        /// [`SagaExecutor::execute_saga_local`]. The tick is **resilient**: a single
        ...
        /// Panics if the internal stats mutex is poisoned (a prior panic occurred
        /// while the lock was held).
        pub async fn run_iteration_local<A: DatabaseAdapter>(
            &self,
        ...
        /// [`SagaExecutor::execute_saga_local`], which transitions it to a terminal
        ...
        /// task that runs [`Self::run_iteration_local`] every `check_interval`. The
        ...
        pub async fn start_background_loop_local<A>(
            self: Arc<Self>,
            ...
            A: DatabaseAdapter + 'static,
        {
            ...
            tokio::spawn(async move {
                ...
                while self.running.load(Ordering::Acquire) {
                    ...
                    if let Err(error) = self.run_iteration_local(executor.as_ref()).await {
