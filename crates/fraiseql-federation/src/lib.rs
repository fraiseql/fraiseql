...
//! | Saga forward execution — `SagaExecutor::{execute_step_local, execute_saga_local, execution_state}` | 🚧 Unstable | requires `unstable-saga` feature; dispatches real mutations (local SQL, or over HTTPS to a registered peer subgraph) and persists real step/saga state (see [#429](https://github.com/fraiseql/fraiseql/issues/429)). A `RetryPolicy` retries a transient step failure with exponential backoff (+ optional per-step timeout) before the saga gives up. A step's caller-supplied `@requires` fields (`RequiredField`) are pre-fetched from their owning subgraph's `_entities` endpoint and merged into the mutation variables before dispatch; an unresolved field fails the step before its mutation runs. |
