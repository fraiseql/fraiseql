...
class FrameworkBenchmarkRunner:
        ...
        try:
            if config.docker_image:
                ...
                async with httpx.AsyncClient() as client:
                    for i in range(30):
                        try:
                            response = await client.get(config.endpoint.replace("/graphql", "/health"))
    ...
    def generate_comparison_report(self, results: List[Dict[str, Any]]):
        ...
        for query_type, query_data in query_results.items():
            ...
            # Table header
            print(f"{'Framework':<20} {'RPS':<10} {'Avg (ms)':<10} {'P95 (ms)':<10} {'P99 (ms)':<10} {'Errors':<10} {'Memory':<10}")
            print("-"*80)
        ...
        fraiseql_results = [r for r in results if r.get("framework") == "FraiseQL" and "error" not in r]
        
        if fraiseql_results:
            print("\n📈 FraiseQL Performance vs Others:")
            
            for query_type in ["simple", "nested", "complex", "mutation"]:
                fraiseql_data = next((r for r in fraiseql_results if r["query"] == query_type), None)
                ...
                for framework in ["Hasura", "PostGraphile", "Strawberry + SQLAlchemy"]:
                    other_data = next((r for r in results if r.get("framework") == framework and r.get("query") == query_type and "error" not in r), None)
                    
                    if other_data:
                        rps_ratio = fraiseql_data["requests_per_second"] / other_data["requests_per_second"]
                        latency_ratio = other_data["avg_latency_ms"] / fraiseql_data["avg_latency_ms"]
                        memory_ratio = fraiseql_data["memory_usage_mb"] / other_data["memory_usage_mb"]
