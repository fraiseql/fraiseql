...
class TurboRouterBenchmark:
    ...
    async def run_turbo_comparison(self):
        ...
        for query_key, query_info in self.test_queries.items():
            ...
            if "error" not in standard_results and "error" not in turbo_results:
                rps_improvement = turbo_results["requests_per_second"] / standard_results["requests_per_second"]
                latency_improvement = standard_results["avg_latency_ms"] / turbo_results["avg_latency_ms"]
                ...
                result = TurboRouterResult(
                    ...
                    actual_query_time_ms=turbo_results["avg_latency_ms"] - overhead["total_overhead_ms"],
                ...
                print(f"\n  Results:")
                print(f"    Standard: {standard_results['requests_per_second']:.1f} RPS, {standard_results['avg_latency_ms']:.2f}ms avg")
                print(f"    TurboRouter: {turbo_results['requests_per_second']:.1f} RPS, {turbo_results['avg_latency_ms']:.2f}ms avg")
    ...
    def generate_detailed_report(self, results: List[TurboRouterResult]):
        ...
        print("-"*80)
        print(f"{'Query':<25} {'Standard RPS':<12} {'Turbo RPS':<12} {'Improvement':<12} {'Avg Latency':<15}")
        print("-"*80)
        ...
        print("\n\n📉 Latency Percentiles")
        print("-"*80)
        print(f"{'Query':<25} {'P95 Standard':<15} {'P95 Turbo':<15} {'P99 Standard':<15} {'P99 Turbo':<15}")
        print("-"*80)
        ...
        simple_improvement = next(r for r in results if r.query_complexity == "simple").rps_improvement
        complex_improvement = next(r for r in results if r.query_complexity == "complex").rps_improvement
                ...
                "results": [
                    {
                        ...
                        "overhead_saved_ms": r.parsing_overhead_ms + r.sql_generation_overhead_ms
                    }
                    for r in results
                ]
            }, f, indent=2)
