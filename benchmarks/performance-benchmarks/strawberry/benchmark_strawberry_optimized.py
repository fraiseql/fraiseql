...
async def make_graphql_request(session: aiohttp.ClientSession, query: str, variables: Dict = None) -> Dict[str, Any]:
...
async def run_query_benchmark(query_name: str, query: str, variables: Dict, 
...
async def check_health() -> bool:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(f"{STRAWBERRY_URL}/health", timeout=aiohttp.ClientTimeout(total=5)) as response:
                if response.status == 200:
                    ...
                    print(f"   Connection Pool: {pool_info.get('size', 0)}/{pool_info.get('max_size', 0)}")
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if result["success"] and result["data"]:
                ...
                if dataloader_stats:
                    print(f"   DataLoader Efficiency:")
                    for loader, data in dataloader_stats.items():
                        efficiency = (data['calls'] / max(1, data['queries'])) if data['queries'] > 0 else 0
                        print(f"     - {loader}: {efficiency:.1f}x (batched {data['calls']} calls into {data['queries']} queries)")
...
async def main():
    ...
    test_configs = [
        ...
        {"query": "projects_deep", "variables": {"statuses": ["planning", "in_progress"], "limit": 10}, "requests": 50},
        {"query": "projects_deep", "variables": {"statuses": ["planning", "in_progress"], "limit": 20}, "requests": 100},
    ...
    if successful_results:
        print(f"{'Query Type':<25} {'Requests':<10} {'Req/s':<10} {'Avg Latency':<12} {'P95 Latency':<12}")
        print("-" * 75)
        ...
        for result in successful_results:
            ...
            print(f"{query_name:<25} {num_requests:<10} {rps:<10.1f} {avg_latency:<12.1f} {p95_latency:<12.1f}")
        ...
        print(f"\n🏆 Best Performance:")
        print(f"   Highest RPS: {best_rps['requests_per_second']:.1f} req/s ({best_rps['query_name']})")
        print(f"   Lowest Latency: {best_latency['avg_latency']:.1f}ms ({best_latency['query_name']})")
