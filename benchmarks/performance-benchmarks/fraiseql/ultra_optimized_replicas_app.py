...
DATABASE_URL_PRIMARY = os.environ.get("DATABASE_URL_PRIMARY", "postgresql://benchmark:benchmark@postgres-primary:5432/benchmark_db")
DATABASE_URL_REPLICAS = os.environ.get("DATABASE_URL_REPLICAS", "postgresql://benchmark:benchmark@pgpool:5432/benchmark_db")
...
class PerformanceMonitor:
    ...
    def record_request(self, pool_type: str = 'read', cache_hit: bool = False, replica_used: bool = False):
    ...
    def get_stats(self):
        return {
            ...
            'replica_percentage': (self.replica_usage['replica'] / max(1, sum(self.replica_usage.values()))) * 100
        }
...
async def get_connection_pools():
    ...
    if not connection_pools:
        ...
        connection_pools['write'] = await asyncpg.create_pool(
            ...
            server_settings={
                'jit': 'off',
                'application_name': 'fraiseql_write_pool'
            }
        )
        ...
        connection_pools['read'] = await asyncpg.create_pool(
            ...
            server_settings={
                'jit': 'off',
                'application_name': 'fraiseql_read_pool'
            }
        )
        ...
        connection_pools['hot'] = await asyncpg.create_pool(
            ...
            server_settings={
                'jit': 'off',
                'application_name': 'fraiseql_hot_pool'
            }
        )
        ...
        if ENABLE_READ_REPLICAS:
            connection_pools['replica'] = await asyncpg.create_pool(
                ...
                server_settings={
                    'jit': 'off',
                    'application_name': 'fraiseql_replica_pool'
                }
            )
...
async def startup_event():
    ...
    pools = await get_connection_pools()
    print(f"✅ Database connection pools initialized:")
    print(f"   - Write pool (primary): {pools['write'].get_min_size()}-{pools['write'].get_max_size()} connections")
    print(f"   - Read pool: {pools['read'].get_min_size()}-{pools['read'].get_max_size()} connections")
    print(f"   - Hot queries pool: {pools['hot'].get_min_size()}-{pools['hot'].get_max_size()} connections")
    if 'replica' in pools:
        print(f"   - Replica pool: {pools['replica'].get_min_size()}-{pools['replica'].get_max_size()} connections")
...
async def benchmark_users(limit: int = 100, response: Response = None):
    ...
    if hot_query_key and hot_query_key in HOT_QUERIES:
        ...
        results = await execute_with_replica_selection(
            "SELECT data FROM tv_users LIMIT $1",
            [limit],
            'read'
        )
        pool_type = "read_pool"
...
async def benchmark_products(limit: int = 100, response: Response = None):
    ...
    if hot_query_key and hot_query_key in HOT_QUERIES:
        ...
        results = await execute_with_replica_selection(
            "SELECT data FROM tv_products LIMIT $1",
            [limit],
            'read'
        )
        pool_type = "read_pool"
...
async def replica_stats():
    ...
    return {
        ...
        'optimization_status': 'read_replicas_active' if ENABLE_READ_REPLICAS else 'read_replicas_disabled'
    }
