...
try:
    ...
    async def check_schema():
        ...
        if not exists:
            ...
            with open('init-db-complex.sql', 'r') as f:
                schema_sql = f.read()
            ...
            print(f"✅ Schema exists with data:")
...
try:
    response = requests.get("http://localhost:8000/benchmark/projects/deep?statuses=planning,in_progress&limit=5")
    result = response.json()
    print(f"   ✅ Query time: {result.get('query_time_ms', 'N/A'):.2f}ms")
    print(f"   ✅ Results: {result.get('result_count', 0)}")
except Exception as e:
...
try:
    ...
    response = requests.post("http://localhost:8000/benchmark/mutations/create-project", json={
