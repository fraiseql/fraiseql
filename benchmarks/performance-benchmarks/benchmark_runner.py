...
def benchmark_service(service_name: str, config: Dict) -> Dict:
    ...
    print(f"{color}{'='*60}{NC}")
    print(f"URL: {url}")
    print(f"Connection: Unix socket (via localhost)")
    ...
    for query_name, query_info in QUERIES.items():
        ...
        if times:
            ...
            # Print summary
            print(f"  {GREEN}✓{NC} Mean: {results[query_name]['mean_ms']:.1f}ms, "
...
def print_comparison(results: Dict):
    ...
    for query_name, query_info in QUERIES.items():
        ...
        for framework, framework_results in results.items():
            if query_name in framework_results and "error" not in framework_results[query_name]:
                ...
                print(f"{query_display:<30} {color}{framework:<12}{NC} "
    ...
    if "FraiseQL" in results and "Strawberry" in results:
        ...
        for query_name in QUERIES.keys():
            if (query_name in results["FraiseQL"] and 
                query_name in results["Strawberry"] and
                "error" not in results["FraiseQL"][query_name] and
                "error" not in results["Strawberry"][query_name]):
                ...
                print(f"\n{query_name}:")
                print(f"  Latency: {winner_color}{latency_ratio:.2f}x{NC} "
                      f"{'faster' if latency_ratio > 1 else 'slower'}")
                print(f"  Throughput: {winner_color}{throughput_ratio:.2f}x{NC} "
...
def main():
    ...
    print(f"  Connection: Unix socket (no network overhead)")
    ...
    if os.path.exists('benchmark_profile.json'):
        with open('benchmark_profile.json', 'r') as f:
