...
class BenchmarkAnalyzer:
    ...
    def generate_comparison_table(self, results: Dict[str, Dict[str, Any]]) -> str:
        ...
        table_data = []
        headers = ["Framework", "Test Type", "Avg Response (ms)", "P95 (ms)", "P99 (ms)", 
    ...
    def generate_markdown_report(self, results: Dict[str, Dict[str, Any]]) -> str:
        ...
        for framework, tests in results.items():
            if 'simple-queries' in tests:
                avg = tests['simple-queries'].get('summary', {}).get('avgResponseTime', float('inf'))
                if avg < best_avg:
                    best_avg = avg
        ...
        report.append(f"- **Highest Throughput**: {best_throughput_framework} ({best_throughput:.1f} req/s)")
        ...
        for framework, tests in results.items():
            ...
            for test_type, data in tests.items():
                ...
                if 'summary' in data:
                    ...
                    report.append(f"- **Average Response Time**: {summary.get('avgResponseTime', 0):.2f}ms")
                    report.append(f"- **P95 Response Time**: {summary.get('p95ResponseTime', 0):.2f}ms")
                    report.append(f"- **P99 Response Time**: {summary.get('p99ResponseTime', 0):.2f}ms")
    ...
    def run(self):
        ...
        report_path = self.reports_dir / f"benchmark_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
        ...
        print(f"- Response time chart: response_time_comparison.png")
        print(f"- Throughput chart: throughput_comparison.png")
        print(f"- Error rate chart: error_rate_comparison.png")
