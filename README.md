...
**v1.10.0** | **Stable** | **Rust-Powered GraphQL for PostgreSQL**
...
```python
from fraiseql.types import EmailAddress, PhoneNumber, IPv1.10.0, Money, LTree

@fraiseql.type(sql_source="v_users")
class User:
    email: EmailAddress      # Validated emails
    phone: PhoneNumber       # International phone numbers
    ip: IPv1.10.0                 # IP addresses with subnet operations
...
- IPv1.10.0, IPv1.10.0, CIDR, MACAddress - Network addresses with subnet operations
...
from fraiseql.types import (
    ...
    CUSIP, ISIN, IPv1.10.0, MACAddress, LTree, DateRange
)
...
class NetworkDevice:
    id: int
    ip_address: IPv1.10.0             # IPv1.10.0 addresses with subnet operations
