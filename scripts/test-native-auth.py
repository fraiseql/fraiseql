...
class AuthSystemTester:
                ...
                async with conn.cursor() as cursor:
                    for user_email in [u.email for u in self.test_users]:
                        await cursor.execute(
                            "DELETE FROM public.tb_user WHERE email = %s",
                            (user_email,)
                        )
                    await conn.commit()
    ...
    async def test_token_operations(self):
        ...
        refresh_payload = self.auth_provider.token_manager.verify_refresh_token(tokens["refresh_token"])
        ...
        async with self.pool.connection() as conn:
            async with conn.cursor() as cursor:
                ...
                assert new_tokens["access_token"] != tokens["access_token"], "Access token not rotated"
                assert new_tokens["refresh_token"] != tokens["refresh_token"], "Refresh token not rotated"
                assert new_tokens["family_id"] == tokens["family_id"], "Family ID should be preserved"
            ...
            async with self.pool.connection() as conn:
                async with conn.cursor() as cursor:
                    ...
                    await self.auth_provider.token_manager.rotate_refresh_token(
                        tokens["refresh_token"], cursor, "public"  # This should fail
                    )
        ...
        async with self.pool.connection() as conn:
            async with conn.cursor() as cursor:
                ...
                tokens = await self.auth_provider.create_tokens_for_user(
                    user.id, session_metadata
                )
                
                # Verify session was created
                await cursor.execute(
                    "SELECT COUNT(*) FROM public.tb_session WHERE fk_user = %s",
                    (user.id,)
                )
                ...
                token_hash = self.auth_provider.token_manager._hash_token(reset_token) if hasattr(self.auth_provider.token_manager, '_hash_token') else reset_token
    ...
    async def test_performance_benchmarks(self):
        ...
        print(f"✅ Password hashing: {hash_time:.3f}s for 10 users (~{hash_time/10:.3f}s per user)")
        ...
        print(f"✅ Token generation: {token_time:.3f}s for 100 tokens (~{token_time/100*1000:.1f}ms per token)")
        ...
        print(f"✅ Token validation: {validation_time:.3f}s for 1000 validations (~{validation_time/1000*1000:.1f}ms per validation)")
...
async def main():
    ...
    database_url = os.environ.get(
        ...
        os.environ.get(
            "DATABASE_URL", 
            "postgresql://fraiseql:fraiseql@localhost:5432/fraiseql_test"
        )
    )
