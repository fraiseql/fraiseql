...
class BlogDataSeeder:
    ...
    async def seed_posts(self, users: List[Dict[str, Any]], count_per_user: int = 2) -> List[Dict[str, Any]]:
        ...
        for i, user in enumerate(users[:3]):  # Only authors create posts
            for j in range(count_per_user):
                # Add random suffix to avoid conflicts
                post_suffix = random.randint(10000, 99999)
                post_data = {
                    "pk_post": str(uuid.uuid4()),
                    "identifier": f"test-post-{post_suffix}-{i+1}-{j+1}",
                    "title": f"Test Blog Post {post_suffix}-{i+1}-{j+1}",
                    "content": f"This is the content for test post {post_suffix}-{i+1}-{j+1}. It contains multiple paragraphs to demonstrate the blog functionality. This is a comprehensive post about testing FraiseQL with real database integration.",
                    "excerpt": f"This is the excerpt for test post {post_suffix}-{i+1}-{j+1}...",
        ...
        async with self.connection.cursor() as cursor:
            for post in posts:
                await cursor.execute("""
                    INSERT INTO tb_post (pk_post, identifier, fk_author, title, content, excerpt, status, published_at, created_at, updated_at)
                    VALUES (%(pk_post)s, %(identifier)s, %(fk_author)s, %(title)s, %(content)s, %(excerpt)s, %(status)s, %(published_at)s, NOW(), NOW())
                """, {
    ...
    async def seed_tags(self, count: int = 8) -> List[Dict[str, Any]]:
        ...
        import random
        tag_names = [
            "FraiseQL", "Testing", "Database", "GraphQL",
            "Python", "PostgreSQL", "Integration", "E2E"
        ]
        
        tags = []
        for i, name in enumerate(tag_names[:count]):
            # Add random suffix to avoid conflicts with existing seed data
            unique_id = f"{name.lower().replace(' ', '-')}-test-{random.randint(1000, 9999)}"
            tag_data = {
    ...
    async def seed_comments(self, posts: List[Dict[str, Any]], users: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for i, post in enumerate(published_posts[:3]):  # Limit to first 3 posts
            for j in range(2):  # 2 comments per post
                ...
                comment_data = {
                    ...
                    "content": f"This is a test comment {j+1} on post '{post['title']}'. Great article, thanks for sharing!",
                    "status": "approved",
        ...
        async with self.connection.cursor() as cursor:
            for comment in comments:
                await cursor.execute("""
                    INSERT INTO tb_comment (pk_comment, fk_post, fk_author, content, status, created_at, updated_at)
                    VALUES (%(pk_comment)s, %(fk_post)s, %(fk_author)s, %(content)s, %(status)s, NOW(), NOW())
                """, {
    ...
    async def associate_posts_with_tags(self, posts: List[Dict[str, Any]], tags: List[Dict[str, Any]]):
