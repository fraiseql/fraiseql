...
1. **Naming Convention Chaos** (24 files affected):
   - Authoritative naming doc (`table-naming-conventions.md`) shows BOTH `users` and `tb_user` without clear guidance
   - Trinity pattern doc (`trinity-identifiers.md`) uses `products` instead of `tb_product`
