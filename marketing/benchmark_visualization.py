...
def create_performance_comparison():
    ...
    for i, (bar1, improvement) in enumerate(zip(bars1, improvements)):
    ...
    ax.set_title('Performance FraiseQL vs Stack Traditionnelle', fontsize=14, fontweight='bold', pad=20)
    ax.set_xticks(x)
...
def create_detailed_benchmark():
    ...
    bars1 = ax1.bar(x - width/2, simple_query, width, label='Requête simple', color='#3498db', alpha=0.8)
    bars2 = ax1.bar(x + width/2, complex_query, width, label='Requête complexe', color='#e74c3c', alpha=0.8)
    ...
    bars = ax2.bar(frameworks_throughput, throughput, color=['#e74c3c', '#f39c12', '#95a5a6', '#3498db'], alpha=0.8)
...
def create_architecture_benefits():
    ...
    categories = ['Performance\n(req/s)', 'Mémoire\n(efficacité)', 'Simplicité\n(lignes de code)', 
                  'Maintenabilité', 'Coût Cloud\n(économies)', 'IA-Friendly']
    ...
    ax.plot(angles, traditional_scores, 'o-', linewidth=2, label='Stack Traditionnelle', color='#95a5a6')
...
def create_token_usage_comparison():
    ...
    bars1 = ax.bar(x - width/2, fraiseql_tokens, width, label='FraiseQL', color='#e74c3c', alpha=0.8)
    bars2 = ax.bar(x + width/2, traditional_tokens, width, label='Stack Traditionnelle', color='#95a5a6', alpha=0.8)
    ...
    ax.text(bars1[-1].get_x() + bars1[-1].get_width()/2., 
    ...
    # Add note
    fig.text(0.5, -0.05, 'Basé sur un blog API avec 5 types, 10 queries, 5 mutations', 
