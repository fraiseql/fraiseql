...
class TestCursorPaginator:
    ...
    async def test_paginate_with_after_cursor(self, db_connection):
        ...
        paginator = CursorPaginator(db_connection)
        params = PaginationParams(first=2, after=encode_cursor("2024-01-02"), order_by="created_at")
    ...
    async def test_paginate_backward(self, db_connection):
        ...
        paginator = CursorPaginator(db_connection)
        params = PaginationParams(last=2, before=encode_cursor("2024-01-04"), order_by="created_at")
        result = await paginator.paginate("v_items", params, include_total=False)
