...
class TestAuditLogQueryBuilderBasics:
    ...
    async def test_builder_initialization(self, sample_events):
    ...
    async def test_recent_operations_basic(self, sample_events):
    ...
    async def test_recent_operations_respects_limit(self, sample_events):
        """recent_operations() respects limit parameter."""
        builder = AuditLogQueryBuilder(sample_events)
        ops = await builder.recent_operations(limit=2)
        assert len(ops) == 2

    async def test_recent_operations_with_type_filter(self, sample_events):
    ...
    async def test_by_user_returns_user_events(self, sample_events):
    ...
    async def test_by_user_respects_time_window(self, sample_events):
    ...
    async def test_by_entity_filters_correctly(self, sample_events):
    ...
    async def test_failed_operations_returns_errors(self, sample_events):
    ...
    async def test_by_event_type_filters(self, sample_events):
    ...
    async def test_by_severity_filters(self, sample_events):
        """by_severity() filters by severity level."""
        builder = AuditLogQueryBuilder(sample_events)
        ops = await builder.by_severity("warning")
        assert all(
            op.metadata.get("severity") == "warning" for op in ops
        )
...
class TestChaining:
    ...
    async def test_filter_by_date_range_chaining(self, sample_events):
    ...
    async def test_limit_returns_self(self, sample_events):
        """limit() returns self for chaining."""
        builder = AuditLogQueryBuilder(sample_events)
        result = builder.limit(10)
        assert result is builder

    async def test_offset_returns_self(self, sample_events):
        """offset() returns self for chaining."""
        builder = AuditLogQueryBuilder(sample_events)
        result = builder.offset(5)
        assert result is builder

    async def test_order_by_returns_self(self, sample_events):
        """order_by() returns self for chaining."""
        builder = AuditLogQueryBuilder(sample_events)
        result = builder.order_by("timestamp")
        assert result is builder

    async def test_filter_by_status_returns_self(self, sample_events):
        """filter_by_status() returns self for chaining."""
        builder = AuditLogQueryBuilder(sample_events)
        result = builder.filter_by_status("error")
        assert result is builder

    async def test_chaining_multiple_filters(self, sample_events):
...
class TestPagination:
    ...
    async def test_limit_constrains_results(self, sample_events):
        """limit() constrains results."""
        builder = AuditLogQueryBuilder(sample_events)
        ops = await builder.limit(2).recent_operations()
        assert len(ops) <= 2

    async def test_offset_skips_results(self, sample_events):
    ...
    async def test_limit_and_offset_together(self, sample_events):
...
class TestAggregations:
    ...
    async def test_count_returns_total(self, sample_events):
        """count() returns total matching events."""
        builder = AuditLogQueryBuilder(sample_events)
        count = await builder.count()
        assert count == len(sample_events)

    async def test_count_with_filter(self, sample_events):
    ...
    async def test_get_statistics_returns_stats(self, sample_events):
    ...
    async def test_statistics_percentiles(self, sample_events):
...
class TestReports:
    ...
    async def test_compliance_report_generation(self, sample_events):
    ...
    async def test_compliance_report_aggregates_correctly(self, sample_events):
    ...
    async def test_compliance_report_includes_failed_ops(self, sample_events):
...
class TestExport:
    ...
    async def test_export_csv(self, sample_events):
    ...
    async def test_export_json(self, sample_events):
...
class TestEdgeCases:
    ...
    async def test_recent_operations_empty_result(self):
    ...
    async def test_by_user_no_match(self, sample_events):
        """by_user() handles non-existent user."""
        builder = AuditLogQueryBuilder(sample_events)
        ops = await builder.by_user("user-nonexistent")
        assert ops == []

    async def test_statistics_with_no_duration(self, sample_events):
    ...
    async def test_compliance_report_summary_string(self, sample_events):
