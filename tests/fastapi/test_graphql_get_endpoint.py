...
class TestGraphQLGetEndpoint:
    ...
    def app_dev(self, clear_registry):
        ...
        return create_fraiseql_app(config=config, queries=[hello, add])
    ...
    def test_get_graphql_with_query_and_variables(self, app_dev):
        ...
        with TestClient(app_dev) as client:
            ...
            response = client.get(
                f"/graphql?query={query}&variables={variables}",
            )
            assert response.status_code == 200
    ...
    def test_get_graphql_with_operation_name(self, app_dev):
        ...
        with TestClient(app_dev) as client:
            ...
            response = client.get(
                f"/graphql?query={query}&operationName=AddQuery",
            )
            assert response.status_code == 200
            assert response.json() == {"data": {"add": 3}}
    ...
    def test_get_graphql_complex_query(self, app_dev):
        ...
        with TestClient(app_dev) as client:
            ...
            response = client.get(f"/graphql?query={query}")
            assert response.status_code == 200
            assert response.json() == {"data": {"result": 12}}
    ...
    def test_get_graphql_url_encoded_query(self, app_dev):
        ...
        with TestClient(app_dev) as client:
            ...
            assert response.status_code == 200
            assert response.json() == {"data": {"hello": "Hello, Test User!"}}
