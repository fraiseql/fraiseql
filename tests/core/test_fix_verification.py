...
class TestJSONBFixVerification:
    ...
    def test_fraiseql_encoder_preserves_json_native_types(self):
        ...
        print("✅ FraiseQLJSONEncoder correctly preserves JSON types while converting PostgreSQL types")
    ...
    def test_end_to_end_graphql_response_simulation(self):
        ...
        assert isinstance(server["ssl_enabled"], bool), f"ssl_enabled should be bool, got {type(server['ssl_enabled'])}"
        assert server["ssl_enabled"] is True, "boolean value should be preserved"
        
        assert isinstance(server["timeout_seconds"], float), f"timeout_seconds should be float, got {type(server['timeout_seconds'])}"
        assert server["timeout_seconds"] == 30.0, "float value should be preserved"
        
        assert isinstance(server["connection_limit"], int), f"connection_limit should be int, got {type(server['connection_limit'])}"
        assert isinstance(server["uptime_percent"], float), f"uptime_percent should be float, got {type(server['uptime_percent'])}"
        assert isinstance(server["maintenance_mode"], bool), f"maintenance_mode should be bool, got {type(server['maintenance_mode'])}"
