...
async def test_json_field_in_mutation():
    ...
    logger.debug(
        "QueryRoot __fraiseql_definition__: %s",
        getattr(QueryRoot, "__fraiseql_definition__", None),
    )
    logger.debug("QueryRoot defined and decorated.")
    logger.debug(
        "Registry types after defining QueryRoot: %s",
        [t.__name__ for t in registry._types],
    )
