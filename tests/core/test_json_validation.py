...
from fraiseql import build_fraiseql_schema, fraise_input, fraise_type
...
class TestJSONValidation:
    ...
    async def test_graphql_mutation_with_invalid_json(self, clear_registry):
        ...
        schema = build_fraiseql_schema(
            ...
            mutation_resolvers=[create_item],
        )
        ...
        result = await graphql(
            schema=schema,
            source=mutation,
            variable_values=valid_variables,
        )
        ...
        result = await graphql(
            schema=schema,
            source=mutation,
            variable_values=invalid_variables,
        )
    ...
    async def test_graphql_query_with_json_literal(self, clear_registry):
        ...
        result = await graphql(
            schema=schema,
            source=query_with_invalid_json,
        )
