...
class TestJSONBNumericCoercionBug:
    ...
    def test_postgresql_jsonb_operator_issue(self):
        ...
        print("ISSUE IDENTIFIED: The SQL generator uses ->> operator which converts all values to TEXT!")
        print("SOLUTION: Use -> operator for type-preserving extraction, then handle JSON serialization properly.")
    ...
    def test_postgresql_types_with_fraiseql_encoder(self):
        ...
        print(f"   - decimal_field: {deserialized['decimal_field']} ({type(deserialized['decimal_field'])})")
