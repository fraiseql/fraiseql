...
class TestJSONBTypeCoercionAdversarial:
    ...
    def test_sql_generator_with_complex_paths(self):
        ...
        complex_field_paths = [
            ...
            FieldPath(alias="settingsTimeout", path=["config", "database", "connection", "timeout"]),
            FieldPath(alias="metricValue", path=["monitoring", "metrics", "cpu", "usage", "percent"]),
        ...
        for field_path in complex_field_paths:
            ...
            if broken_sql != expected_sql:  # Only check if there was a replacement
                assert broken_sql not in sql_str, (
                    f"Found broken ->> operator in SQL: {broken_sql}"
                )
    ...
    def test_graphql_response_structure_adversarial(self):
        ...
        for i, server in enumerate(cluster["servers"]):
            assert isinstance(server["port"], int), f"Server {i} port should be int, got {type(server['port'])}"
            ...
            assert isinstance(server["uptime_seconds"], int), f"Server {i} uptime_seconds should be int"
            ...
            assert all(isinstance(rt, int) for rt in metrics["response_times"]), "Response times should be array of ints"
    ...
    def test_typescript_interface_compatibility(self):
        ...
        assert all(isinstance(tag, str) for tag in server["tags"]), "tags array must contain only strings"
        assert all(isinstance(rt, int) for rt in server["metrics"]["responseTimeMs"]), "responseTimeMs must be number[]"
