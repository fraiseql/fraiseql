...
class TestDualModeRepositoryUnit:
    ...
    def test_camel_to_snake_case_conversion(self, mock_pool):
        ...
        result = repo._instantiate_recursive(TestType, data)
    ...
    def test_build_find_query(self, mock_pool):
        ...
        assert "SELECT * FROM tv_product" in str(query.statement)
        assert query.params == {}
        assert query.fetch_result is True

        # Test with parameters
        query = repo._build_find_query("tv_product", id=uuid4(), status="available")
        assert "SELECT * FROM tv_product" in str(query.statement)
        assert "WHERE" in str(query.statement)
        assert "id" in query.params
        assert "status" in query.params
        assert query.params["status"] == "available"
    ...
    def test_build_find_one_query(self, mock_pool):
        ...
        assert "SELECT * FROM tv_product" in str(query.statement)
        assert "LIMIT 1" in str(query.statement)
        ...
        assert "WHERE" in str(query.statement)
        assert "LIMIT 1" in str(query.statement)
        assert query.params["id"] == product_id
