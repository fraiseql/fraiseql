...
from ..common.exceptions import BusinessRuleViolationError, DomainValidationError
from .value_objects import ContactEmail, OrganizationIdentifier, OrganizationName
...
class SubscriptionPlan(Enum):
    ...
    def can_upgrade_to(self, other: "SubscriptionPlan") -> bool:
        ...
        hierarchy = {self.STARTER: 0, self.PROFESSIONAL: 1, self.ENTERPRISE: 2, self.CUSTOM: 3}
...
class Organization(AggregateRoot):
    ...
    def upgrade_subscription(self, new_plan: SubscriptionPlan) -> None:
        ...
        if self.status == OrganizationStatus.CANCELLED:
            raise BusinessRuleViolationError(
                "Cannot upgrade subscription for cancelled organization"
            )
    ...
    def update_contact_info(
        self, contact_email: ContactEmail | None = None, website_url: str | None = None
    ) -> None:
