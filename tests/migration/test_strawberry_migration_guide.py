...
class TestMigrationGuideExists:
    ...
    def test_migration_guide_has_code_examples(self):
        ...
        strawberry_examples = len(re.findall(r'```python.*?strawberry', content, re.DOTALL | re.IGNORECASE))
        fraiseql_examples = len(re.findall(r'```python.*?fraiseql', content, re.DOTALL | re.IGNORECASE))
...
class TestStrawberryCompatibilityLayer:
    ...
    def test_strawberry_style_field_resolver_migration(self):
        ...
        with TestClient(app) as client:
            ...
            data = response.json()
            assert "data" in data
            assert data["data"]["get_user"]["display_name"] == "User: John"
...
class TestStrawberryFeatureParity:
    ...
    def test_strawberry_enum_migration(self):
        ...
        with TestClient(app) as client:
            ...
            data = response.json()
            assert "data" in data
            assert data["data"]["get_user"]["role"] == "ADMIN"  # FraiseQL uses enum name
