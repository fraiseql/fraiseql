...
class TestAdaptiveAuthValidation:
    ...
    def test_jwt_expiration_scales_correctly(self, profile) -> None:
    ...
    @pytest.mark.parametrize("profile", ["low", "medium", "high"])
    def test_jwt_signature_validation_scales_correctly(self, profile) -> None:
    ...
    @pytest.mark.parametrize("profile", ["low", "medium", "high"])
    def test_config_timeout_scaling(self, profile) -> None:
        ...
        if profile == "low":
            ...
            assert 2.0 <= config.timeout_seconds <= 7.0, (
                "MEDIUM profile should have moderate timeouts"
            )
        elif profile == "high":
    ...
    @pytest.mark.parametrize("profile", ["low", "medium", "high"])
    def test_concurrent_requests_scaling(self, profile) -> None:
        ...
        if profile == "low":
            assert config.concurrent_requests == 50, (
                f"LOW profile should have 50 concurrent requests, got {config.concurrent_requests}"
            )
        elif profile == "medium":
            assert config.concurrent_requests == 100, (
                f"MEDIUM profile should have 100 concurrent requests, got {config.concurrent_requests}"
            )
        elif profile == "high":
            assert config.concurrent_requests == 400, (
                f"HIGH profile should have 400 concurrent requests, got {config.concurrent_requests}"
            )
    ...
    def test_multiplier_based_formula_never_breaks(self) -> None:
    ...
    def test_divisor_based_formula_would_fail(self) -> None:
        ...
        for profile in profiles:
            ...
            if profile == "low":
                ...
                assert bad_iterations == 2, (
                    "MEDIUM profile divisor-based formula produces only 2 iterations"
                )
            elif profile == "high":
                # HIGH: 400 // 40 = 10 iterations (this works, but others don't!)
                assert bad_iterations == 10, (
                    "HIGH profile divisor-based formula works, but LOW/MEDIUM fail"
                )

            # Show that multiplier-based is always better
            good_iterations = max(5, int(10 * config.load_multiplier))
            assert good_iterations >= 5, (
                f"Multiplier-based always produces meaningful iterations: {good_iterations}"
            )
            ...
            if profile == "low":
                ...
                assert improvement == 5.0, (
                    f"Multiplier-based is {improvement}x better on LOW profile"
                )
