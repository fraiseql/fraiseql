...
##############################################################################
# 2-Subgraph Federation Integration Test Runner
#
# This script starts the Docker Compose environment, runs the 2-subgraph
# federation tests, and provides a summary of results.
#
# Usage:
#   ./run_2subgraph_tests.sh [--no-cleanup]
#
# Options:
...
# Main test execution
main() {
    log_header "2-Subgraph Federation Integration Tests"
    echo
    ...
    while [ $WAIT_TIME -lt $MAX_WAIT ]; do
        ...
        TOTAL_SERVICES=4  # postgres-users, postgres-orders, postgres-products, users-subgraph, orders-subgraph, products-subgraph, apollo-router
    ...
    # Step 4: Run tests
    log_header "Running 2-Subgraph Federation Tests"
    echo

    cd "$PROJECT_ROOT"

    # Collect test results
    TEST_RESULTS_FILE="/tmp/federation_test_results_$$.txt"
    ...
    if [ $CARGO_EXIT_CODE -eq 0 ]; then
        log_success "All 2-subgraph tests passed!"
    ...
    if [ $CARGO_EXIT_CODE -eq 0 ]; then
        log_success "2-Subgraph federation integration tests completed successfully!"
    else
        log_error "2-Subgraph federation integration tests failed!"
    fi
