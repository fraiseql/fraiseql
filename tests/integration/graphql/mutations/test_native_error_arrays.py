...
NOTE: These tests are currently skipped due to architectural constraints.
See /tmp/fraiseql-phase1.5-blocker-analysis.md for details.

BLOCKER: GraphQL schema is built before test fixtures run, preventing dynamic
mutation discovery. Requires framework enhancement for dynamic schema refresh.
"""
...
@pytest.mark.skip(
    reason="WP-034: Requires dynamic schema discovery - GraphQL schema is built before "
           "test fixtures can create database functions. See /tmp/fraiseql-phase1.5-blocker-analysis.md. "
           "Feature can be manually tested - automated tests deferred to Phase 5 (schema refresh enhancement)."
)
class TestAutoGeneratedErrors:
    ...
    async def test_auto_generated_errors_from_status(self, blog_simple_graphql_client):
        ...
        Prerequisites (BLOCKED - see class decorator):
        - Requires dynamic function creation before schema init
        - Currently not possible with fixture architecture
        """
        # Note: This test is skipped - see class-level skip decorator
        # Execute mutation
        query = """
            mutation {
                testAutoError {
        ...
        # ✅ THIS IS THE KEY TEST - errors array should exist and be populated
    ...
    async def test_auto_generated_errors_multiple_status_formats(
        self, blog_simple_graphql_client
    ):
        """
        ...
        for status, expected_id, mutation_name in test_cases:
            ...
            assert result.get("errors") is None, f"GraphQL errors for {status}: {result.get('errors')}"
...
@pytest.mark.skip(
    reason="WP-034: Requires dynamic schema discovery - see /tmp/fraiseql-phase1.5-blocker-analysis.md"
)
class TestExplicitErrors:
    """Test explicit error arrays via metadata.errors."""

    @pytest.mark.asyncio
    async def test_explicit_errors_override_auto_generation(self, blog_simple_graphql_client):
        """
