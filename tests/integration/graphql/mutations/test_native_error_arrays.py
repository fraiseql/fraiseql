...
WP-034 Phase 2 - Tests BLOCKED pending mutation_response mapping investigation

CURRENT STATUS: Schema refresh API (Phases 1-3) is complete and working.
These tests are blocked on understanding how to properly map PostgreSQL
mutation_response composite types to GraphQL mutation types in FraiseQL v1.8.

The database functions return mutation_response, but creating Python wrappers
that return the correct GraphQL structure (with code, status, message, errors fields)
requires investigation into FraiseQL's mutation processing layer.
...
@pytest.mark.skip(reason="WP-034: Blocked on mutation_response -> GraphQL type mapping")
