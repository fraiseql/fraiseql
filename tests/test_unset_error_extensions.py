...
@fraiseql.query
async def error_query_func(info) -> TestType:
...
def test_graphql_error_with_unset_in_extensions(test_client):
...
def test_mutation_error_with_unset_input(test_client, mocker):
...
def test_production_mode_error_handling(mocker):
