    ...
    async def test_create_author_identifier_slug_validation(self, graphql_client):
        ...
        for invalid_id in invalid_identifiers:
            ...
            if result["data"]["createAuthor"]["__typename"] == "CreateAuthorError":
                ...
                assert "identifier" in error["message"].lower() or "invalid" in error["message"].lower()
    ...
    async def test_create_post_content_security_validation(self, graphql_client):
        ...
        for dangerous_content, expected_error in dangerous_content_tests:
            ...
            if result["data"]["createPost"]["__typename"] == "CreatePostError":
                ...
                assert "security" in error["message"].lower() or "invalid" in error["message"].lower()

    async def test_create_post_with_complex_tag_hierarchy(self, graphql_client):
        """Test post creation with hierarchical tags."""
        # Create author
        await graphql_client.create_author(
            identifier="tag-hierarchy-author",
            name="Tag Hierarchy Author",
...
class TestErrorMetadataEnhancements:
    ...
    async def test_error_response_consistency_across_mutations(self, graphql_client, clean_database):
...
class TestDatabaseTransactionIntegrity:
    ...
    async def test_failed_mutation_does_not_leave_partial_data(self, graphql_client, clean_database, db_connection):
        ...
        post_count = await db_connection.fetchval("SELECT COUNT(*) FROM blog.tb_post WHERE identifier = $1", "transaction-test-post")
        assert post_count == 0

        # Verify no tag associations were created
        tag_count = await db_connection.fetchval("SELECT COUNT(*) FROM blog.tb_post_tag pt JOIN blog.tb_post p ON pt.fk_post = p.pk_post WHERE p.identifier = $1", "transaction-test-post")
        assert tag_count == 0

        # Verify materialized table wasn't updated
        tv_post_count = await db_connection.fetchval("SELECT COUNT(*) FROM tv_post WHERE identifier = $1", "transaction-test-post")
...
class TestCacheInvalidationPatterns:
    ...
    async def test_author_post_count_updates_on_post_creation(self, graphql_client, clean_database, db_connection):
        ...
        initial_count = await db_connection.fetchval(
            "SELECT post_count FROM tv_author WHERE id = $1",
            author_id
        )
    ...
    async def test_materialized_table_consistency_after_operations(self, graphql_client, clean_database, db_connection):
        ...
        materialized_data = await db_connection.fetchrow(
            "SELECT id, identifier, data FROM tv_post WHERE id = $1",
            post_id
        )
...
class TestPerformanceCharacteristics:
    ...
    async def test_bulk_author_creation_performance(self, graphql_client, clean_database):
        ...
        print(f"Created 50 authors in {total_time:.2f} seconds ({avg_time_per_author:.3f}s avg per author)")
    ...
    async def test_complex_post_creation_performance(self, graphql_client, clean_database):
        ...
        result = await graphql_client.create_post(
            ...
            content="This is a complex post with multiple tags for performance testing. " * 100,  # Longer content
        ...
        print(f"Created complex post with {len(tag_identifiers)} tags in {execution_time:.3f} seconds")
