...
def test_select_with_where_clause():
    ...
    query = build_sql_query(
        table="users",
        field_paths=[
            FieldPath(alias="email", path=["contact", "email"]),
        ],
        where_clause=where,
    )
...
def test_nested_path_multiple_levels():
    query = build_sql_query(
        table="events",
        field_paths=[
            FieldPath(alias="city", path=["location", "address", "city"]),
        ],
    )
...
def test_group_by_single_field():
    ...
    query = build_sql_query(
        table="orders",
        field_paths=[
            FieldPath(alias="status", path=["status"]),
        ],
        group_by=["status"],
    )
...
def test_group_by_deeply_nested():
    ...
    query = build_sql_query(
        table="events",
        field_paths=[
            FieldPath(alias="venue", path=["location", "venue", "name"]),
        ],
...
def test_complex_nested_scenario():
    ...
    query = build_sql_query(
        ...
        group_by=[
            "user.device.browser.name",
            "geo.location.country",
            "user.device.os.version",
        ],
        order_by=[
            ("geo.location.country", "asc"),
            ("user.device.browser.name", "desc"),
        ],
    )
