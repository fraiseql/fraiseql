-- Federation CI MySQL initialization
-- Creates both databases and grants the fraiseql user access to both.

CREATE DATABASE IF NOT EXISTS fraiseql_inventory;
GRANT ALL PRIVILEGES ON fraiseql_inventory.* TO 'fraiseql'@'%';
FLUSH PRIVILEGES;

-- Orders Service Tables (Trinity Pattern) — in fraiseql database (default)
USE fraiseql;
...
SELECT pk_order, JSON_OBJECT('id', id, 'user_id', user_id, 'status', status,
    'total', total, 'created_at', created_at, 'updated_at', updated_at) AS data
FROM tb_order;

CREATE VIEW v_order_item AS
SELECT pk_order_item, JSON_OBJECT('id', id, 'product_id', product_id,
    'quantity', quantity, 'price', price) AS data
FROM tb_order_item;
...
SELECT pk_product, JSON_OBJECT('id', id, 'name', name, 'stock', stock,
    'price', price, 'created_at', created_at, 'updated_at', updated_at) AS data
FROM tb_product;

CREATE VIEW v_reservation AS
SELECT pk_reservation, JSON_OBJECT('id', id, 'order_id', order_id,
    'status', status, 'created_at', created_at, 'updated_at', updated_at) AS data
FROM tb_reservation;

CREATE VIEW v_reservation_item AS
SELECT pk_reservation_item, JSON_OBJECT('id', id, 'product_id', product_id,
    'quantity', quantity) AS data
