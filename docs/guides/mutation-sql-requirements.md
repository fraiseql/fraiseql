...
```sql
result.status := 'failed:validation';   -- Field 1
...
**You DO set in SQL:**
- ✅ `status` - Use format `prefix:identifier` (e.g., `"failed:validation"`)
...
2. Extracts `"validation"` from `"failed:validation"`
...
Use status strings like `"failed:validation"` or `"not_found:user"`. FraiseQL extracts the identifier and creates structured errors automatically.
...
**GraphQL Response:** Errors array auto-populated from `"failed:validation"` → `{"identifier": "validation"}`.
...
-- Error states
SELECT mutation_validation_error('Invalid input'); -- → status: 'failed:validation'
...
| `failed:validation` | 422 | Input validation | Required fields missing |
