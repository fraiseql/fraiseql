...
1. **Client provides identifier** → Look up `pk_*` first
2. **Client provides UUID** → Look up `pk_*` first
3. **Internal queries** → Use `pk_*` directly
...
3. **Make `identifier` nullable if not always needed**
   ...
   ```sql
   identifier TEXT UNIQUE  -- ✅ NULL for internal records
   ```

4. **Use meaningful identifier slugs**

   ```sql
   identifier = 'john-doe-engineering'  -- ✅ Context + uniqueness
   ```
...
1. **Don't use UUID for foreign keys**
   ...
   ```sql
   user_id UUID REFERENCES tb_user(id)  -- ❌ Slow!
   ```

2. **Don't expose `pk_*` in GraphQL**

   ```python
   pk_user: int  -- ❌ Internal only!
   ```

3. **Don't use sequential integers as public IDs**

   ```python
   id: int  -- ❌ Enumeration attack risk!
   ```
...
✅ **Stability** - UUID IDs for public API
✅ **Usability** - Text identifiers for human-friendly URLs
✅ **Security** - Non-sequential UUIDs prevent enumeration
✅ **Flexibility** - Choose the right identifier for each use case
