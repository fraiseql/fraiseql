...
```sql
<!-- Code example in SQL -->
-- PostgreSQL (following FraiseQL Naming Patterns)
CREATE TABLE tb_user (                           -- ✅ Singular table name
  pk_user BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY,  -- ✅ BIGINT not SERIAL
  id UUID NOT NULL UNIQUE,                      -- ✅ UUID v4 for GraphQL ID
...
When you define `NOT NULL` in the database, FraiseQL's GraphQL schema will correctly reflect that the field is non-nullable. You don't need to specify it twice. See [Naming Patterns](../reference/naming-patterns.md) for why `tb_user` (singular), `pk_user` (BIGINT), and `id` (UUID) are required.

**2. Database performance directly translates to API performance**

```sql
<!-- Code example in SQL -->
-- Adding a database index (using correct table name)
CREATE INDEX idx_user_email ON tb_user(email);  -- ✅ Singular table name
```text
<!-- Code example in TEXT -->
...
@schema.type(table="tb_user")           # ✅ Singular table name
class User:
...
query GetUser($id: ID!) {              # ✅ ID scalar for entity identifiers
  user(id: $id) {
    id
    email
  }
...
# ❌ BAD: Wrong table name, wrong ID type
@schema.type(table="tb_user")          # ❌ Plural (wrong)
class User:
    user_id: str                        # ❌ Should be id: UUID
    email: str

# ✅ GOOD: Follows FraiseQL naming patterns
@schema.type(table="tb_user")           # ✅ Singular (correct)
class User:
    id: UUID                            # ✅ UUID v4 (correct)
    email: str                          # ✅ Other fields match database
```text
<!-- Code example in TEXT -->

See [Naming Patterns](../reference/naming-patterns.md) for why singular table names and UUID identifiers are required.
...
```sql
<!-- Code example in SQL -->
-- Foreign keys define relationships (following Naming Patterns)
ALTER TABLE tb_order                            -- ✅ Singular
  ADD CONSTRAINT fk_order_user                  -- ✅ Singular
  FOREIGN KEY (fk_user) REFERENCES tb_user(pk_user);  -- ✅ Correct naming
```text
<!-- Code example in TEXT -->

FraiseQL uses these foreign keys to automatically enable GraphQL relationships. No custom resolver needed. See [Naming Patterns](../reference/naming-patterns.md#2-pk--fk-bigint-internal-keys) for pattern conventions.
