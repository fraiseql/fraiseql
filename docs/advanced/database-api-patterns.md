...
```sql
-- Table view for expensive aggregations
...
BEGIN
    -- Upsert user statistics with fresh calculations
    INSERT INTO tv_user_statistics
    SELECT
        u.id,
...
BEGIN
    ...
    RETURN ROW(
        ...
        jsonb_build_object('id', v_post_id)
    )::mutation_result;
END;
$$ LANGUAGE plpgsql;
...
async def create_post(input: CreatePostInput, context) -> CreatePostSuccess | CreatePostError:
    ...
    if result['success']:
        ...
        post_data = await context.db.query_one(
            "SELECT data FROM v_post WHERE id = $1",
            result['data']['id']
        )
        return CreatePostSuccess(post=Post.from_dict(post_data))
    else:
        return CreatePostError(message=result['message'])
```
...
CREATE OR REPLACE FUNCTION sync_user_statistics_batch(
    p_user_ids UUID[]
) RETURNS void AS $$
BEGIN
    ...
    SELECT
        u.id,
        COALESCE(stats.post_count, 0),
        COALESCE(stats.comment_count, 0),
        COALESCE(stats.follower_count, 0),
        COALESCE(stats.following_count, 0),
        COALESCE(stats.engagement_score, 0),
        NOW(),
        stats.data
    FROM tb_users u
    LEFT JOIN (
        -- Calculate stats for all users in one query
        SELECT
            u.id as user_id,
            ...
            jsonb_build_object(
                ...
                'follower_count', COUNT(DISTINCT fr.follower_id),
                'following_count', COUNT(DISTINCT fg.followed_id)
            ) as data
        FROM tb_users u
        LEFT JOIN tb_posts p ON p.author_id = u.id
        LEFT JOIN tb_comments c ON c.author_id = u.id
        LEFT JOIN tb_follows fr ON fr.followed_id = u.id
        LEFT JOIN tb_follows fg ON fg.follower_id = u.id
