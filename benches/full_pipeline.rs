    ...
    c.bench_function("simple_query", |b| {
        ...
        b.to_async(tokio::runtime::Runtime::new().unwrap()).iter(|| async {
            let result = fraiseql_rs::execute_graphql_query(
                ...
                black_box(pyo3::types::PyDict::new(pyo3::Python::acquire_gil().python())),
                black_box(pyo3::types::PyDict::new(pyo3::Python::acquire_gil().python())),
            )
            .await;
            black_box(result)
...
criterion_group!(benches, benchmark_simple_query, benchmark_complex_where, benchmark_cached_query);
