...
if [ -z "${DATABASE_URL:-}" ]; then
    ...
    echo "  psql -v ON_ERROR_STOP=1 -d fraiseql_example -f ../basic/sql/setup.sql" >&2
    echo "  export DATABASE_URL=postgresql://localhost/fraiseql_example" >&2
    exit 2
fi

cargo run --quiet -p fraiseql-cli -- \
    compile "../basic/schema.json" -o "../basic/schema.compiled.json"

exec cargo run --quiet -p fraiseql-example-error-handling
