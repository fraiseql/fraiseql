"""DataLoaders for the blog API to prevent N+1 queries."""
...
class UserDataLoader(DataLoader[UUID, Dict[str, Any]]):
    ...
    def __init__(self, db: BlogRepository):
        super().__init__()
        self.db = db
...
class CommentsByPostDataLoader(DataLoader[UUID, List[Dict[str, Any]]]):
    ...
    async def batch_load(self, post_ids: List[UUID]) -> List[List[Dict[str, Any]]]:
        ...
        for comment in all_comments:
            # Handle both postId (camelCase) and post_id (snake_case)
            post_id_str = comment.get("postId") or comment.get("post_id")
            if post_id_str:
