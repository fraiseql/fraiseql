...
class TestRedPhaseMultipleValidationErrorArrays:
    ...
    async def test_create_author_mixed_validation_types_returns_structured_array(
        self, graphql_client
    ):
    ...
    async def test_create_post_comprehensive_validation_array_with_security_errors(
        self, graphql_client
    ):
        ...
        dangerous_content = (
            '<script>alert("xss")</script>Content with javascript:void(0) and ../../../etc/passwd'
        )
        ...
        assert (
            len(errors) >= 8
        )  # identifier, title, content length, security x3, author, tags x2, status
...
class TestRedPhaseErrorArrayStructure:
    ...
    async def test_error_array_structure_follows_printoptim_patterns(
        ...
        for i, error in enumerate(errors):
            ...
            assert "_" in error["identifier"], (
                f"Error {i} identifier should be snake_case: {error['identifier']}"
            )
...
class TestRedPhaseSecurityValidationArrays:
    ...
    async def test_multiple_security_violations_in_structured_array(
        self, graphql_client, clean_database
    ):
...
class TestRedPhasePerformanceWithErrorArrays:
    ...
    async def test_many_validation_errors_handled_efficiently(self, graphql_client, clean_database):
        ...
        assert execution_time < 5.0, (
            f"Should handle many errors efficiently, took {execution_time:.2f}s"
        )
