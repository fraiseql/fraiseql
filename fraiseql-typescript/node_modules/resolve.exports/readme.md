...
* an `entry` *is* defined but no known conditions were matched (see [`options.conditions`](#optionsconditions))
...
* an `target` *is* defined but no known conditions were matched (see [`options.conditions`](#optionsconditions))
...
> **Note:** Although the Node.js documentation primarily showcases conditions alongside `"exports"` usage, they also apply to `"imports"` maps too. *([example](https://nodejs.org/docs/latest-v18.x/api/packages.html#subpath-imports))*
